...
export class StaffService {
    ...
    async addStaff(staffData:StaffModel) {
        try{
            ...
            const staffFieldDocs = await Field.find({code: {$in:staffData.fields}}).lean<{_id:mongoose.Types.ObjectId}[]>();
            staffFieldCode = staffFieldDocs.map((field)=>field._id);

            const staffLogDocs = await Log.find({code: {$in: staffData.logs}}).lean<{_id:mongoose.Types.ObjectId}[]>();
            staffLogCode = staffLogDocs.map((log)=>log._id);

            const staffVehicleDocs = await Vehicle.find({
                licensePlateNumber: { $in: staffData.vehicles } // ✅ Correct field name
            }).lean<{ _id: mongoose.Types.ObjectId, licensePlateNumber: string }[]>();
            
            staffVehicleNumber = staffVehicleDocs.map((vehicle) => vehicle._id);

            const staffEquipmentDocs = await Equipment.find({id: {$in: staffData.equipments}}).lean<{_id:mongoose.Types.ObjectId}[]>();
