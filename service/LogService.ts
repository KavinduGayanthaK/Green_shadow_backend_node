...
export class LogService {
  ...
  async generateLogCode(): Promise<string> {
    try {
      ...
      if (lastLog?.logCode) {
        const lastIdNumber = parseInt(lastLog.logCode.replace("LOG-", ""), 10);
  ...
  async addLog(logData: LogModel) {
    try {
      ...
      const getFields = await this.fieldRepository.getSelectedField(
        savedLog.logFields
      );
  ...
  async updateLog(id: string, logData: LogModel) {
    try {
      ...
      const result = await this.logRepository.updateLog(
        logData.logCode,
        updateData
      );
      await this.fieldRepository.updateFieldAssignLog(logData.logCode, logData);
