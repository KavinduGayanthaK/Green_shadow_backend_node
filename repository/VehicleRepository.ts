...
export class VehicleRepository {
  ...
  async deleteVehicle(code: string) {
    try {
      const result = await Vehicle.deleteOne({ code });
      return result
  ...
  async updateVehicleAssignStaff(code: string, staffData: StaffModel) {
    try {
      const staffDoc = await Staff.findOne({ staffId: code }).lean<{
      ...
      const existingVehicleDocs = await Vehicle.find({
        vehicleStaffMember: staffId,
      }).lean<{ _id: mongoose.Types.ObjectId }[]>();
      const existingVehicleIds = existingVehicleDocs.map(
        (vehicle) => vehicle._id
      );

      // Get updated list of vehicles from staffData
      const updatedVehicleDocs = await Vehicle.find({
        licensePlateNumber: { $in: staffData.vehicles },
      }).lean<{ _id: mongoose.Types.ObjectId }[]>();
      const updatedVehicleIds = updatedVehicleDocs.map(
        (vehicle) => vehicle._id
      );

      // Find vehicles to remove staff from
      const vehiclesToRemove = existingVehicleIds.filter(
        (id) => !updatedVehicleIds.includes(id)
      );
      const vehiclesToAdd = updatedVehicleIds.filter(
  ...
  async findVehicleById(code: string): Promise<IVehicle | null> {
    return await Vehicle.findOne({ licensePlateNumber: code })
      .populate("vehicleStaffMember")
      .exec();
  }
}
